...
public class GalleryRepository {
    ...
    private MutableLiveData<List<GalleryMediaBean>> medialist;
            ...
            new Thread() {
                ...
                public void run() {
                    ...
                    handler.post(new Runnable() {
                        @Override
                        public void run() {
                            List<GalleryMediaBean> GalleryMediaBeanList = new ArrayList<>();
                            ...
                            for (int i = 0; i < count; i++) {
                                ...
                                GalleryMediaBean multimediaItem = new GalleryMediaBean();

                                if (type == MediaStore.Files.FileColumns.MEDIA_TYPE_IMAGE) {
                                    multimediaItem.setMediaType(type);
...
//                               multimediaItem.setPath(AppUtils.getRealPathFromURI(application,multimediaItem.getUri()));

                                GalleryMediaBeanList.add(multimediaItem);
                            }

                            cursor.close();

                            medialist.postValue(GalleryMediaBeanList);
                        }
                    });
                    Looper.loop();
                }
   ...
   /* private void getBucketsList(Context context){
        ...
        bucketBean.setBucketName(context.getString(R.string.txt_gallery));
...
*/
     ...
     * @return
     */
    private List<GalleryMediaBean> getGalleryItems(Context context) {
...
*/
        ...
        cursor.close();
        medialist.postValue(GalleryMediaBeanList);
        return GalleryMediaBeanList;

    }

    public MutableLiveData<List<GalleryMediaBean>> getMedialist() {
        return medialist;
    }
